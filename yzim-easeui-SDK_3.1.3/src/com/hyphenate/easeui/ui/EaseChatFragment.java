...
import com.yz.im.utils.ClipBoardUtil;
...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    protected void initView() {
        ...
        inputMenu.setChatInputMenuListener(new ChatInputMenuListener() {
            ...
            @Override
            public void onBigExpressionClicked(EaseEmojiIcon emojicon) {
                //发送大表情(动态表情)
    ...
    protected void setUpView() {
        ...
        if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
            // 设置标题
            String title = EaseUserUtils.getUserName(IdOffsetUtil.minusOffset(toChatUsername));
    ...
    private void setGroupTitleName() {
        Group group = IMHelper.getInstance().getGroupInfo(toChatUsername, true);
        if (group != null) {
            ...
            setTitleFromExtraAttribute(MessageExtraAttribute.KEY_GROUP_NAME);
        }
    }

    private void setTitleFromExtraAttribute(String key) {
        EMConversation conversation = EMClient.getInstance().chatManager().getConversation(toChatUsername);
        if (conversation != null) {
            EMMessage message = conversation.getLastMessage();
            if (message != null) {
                try {
                    titleBar.setTitle(message.getStringAttribute(key));
    ...
    protected void setListItemClickListener() {
        messageList.setItemClickListener(new EaseChatMessageList.MessageListItemClickListener() {
            ...
            @Override
            public void onDialogItemClick(EMMessage message, String tag, int position) {
                switch (tag) {
                    case "复制":
                        new ClipBoardUtil(getContext()).setText(((EMTextMessageBody) message.getBody()).getMessage());
                        break;
                    case "删除":
                        ...
                        break;
                    case "转发":
                        IMHelper.getInstance().getNavigator().navigateToFriendTransferActivity(getContext(), message);
                        break;
                }
            }
    ...
    public void onMessageReceived(List<EMMessage> messages) {
        for (EMMessage message : messages) {
            ...
            if (username.equals(toChatUsername)) {
                ...
                // 声音和震动提示有新消息
//                EaseUI.getInstance().getNotifier().variableAndPlayTone(message);
            } else {
                // 如果消息不是和当前聊天ID的消息
//                EaseUI.getInstance().getNotifier().onNewMsg(message);
    ...
    protected void inputAtUsername(String username, boolean autoAddAtSymbol) {
        ...
        Friend friend = IMHelper.getInstance().getFriendInfo(IdOffsetUtil.minusOffset(username));
        if (friend != null) {
            if (autoAddAtSymbol)
                inputMenu.insertText("@" + friend + " ");
            else
                inputMenu.insertText(friend + " ");
        } else {
            if (autoAddAtSymbol)
    ...
    protected void sendUserCardMessage(String content) {
        EMMessage message = EMMessage.createTxtSendMessage(content, toChatUsername);
        message.setAttribute(EaseConstant.MESSAGE_ATTR_IS_USR_CARD, true);
        sendMessage(message);
    }
    ...
    public interface EaseChatFragmentHelper {
         ...
         * 消息气泡框长按事件
         */
        boolean onMessageBubbleLongClick(EMMessage message);
