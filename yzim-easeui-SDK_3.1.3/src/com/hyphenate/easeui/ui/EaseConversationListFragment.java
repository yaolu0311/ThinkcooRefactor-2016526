...
public class EaseConversationListFragment extends EaseBaseFragment {
        ...
        Collections.sort(conversationList, new Comparator<Pair<Long, EMConversation>>() {
            ...
            public int compare(final Pair<Long, EMConversation> con1, final Pair<Long, EMConversation> con2) {
                ...
                if (leftExtValue == rightExtValue) {
                    result = compareByTime(con1, con2);
    ...
    private int compareByTime(Pair<Long, EMConversation> con1, Pair<Long, EMConversation> con2) {
    ...
    private void setExtValue(EMConversation conversation) {
        ...
        if (conversation.getType() == EMConversation.EMConversationType.Chat) {
            Friend friend = getFriendCache().getEntity(IdOffsetUtil.minusOffset(conversation.getUserName()));
            if (friend == null) {
                conversation.setExtField(STICK_NORMAL);
            } else {
                String stick = friend.getStick();
                conversation.setExtField(TextUtils.isEmpty(stick) ? STICK_NORMAL : stick);
            }
        } else if (conversation.getType() == EMConversation.EMConversationType.GroupChat) {
            Group group = getGroupCache().getEntityByDiffId(conversation.getUserName(), true);
            if (group == null) {
                conversation.setExtField(STICK_NORMAL);
            } else {
                String stick = group.getStick();
                conversation.setExtField(TextUtils.isEmpty(stick) ? STICK_NORMAL : stick);
            }
        }
    ...
    private FriendCache getFriendCache() {
        return FriendCache.getInstance(getContext());
    }

    private GroupCache getGroupCache() {
        return GroupCache.getInstance(getContext());
    }
