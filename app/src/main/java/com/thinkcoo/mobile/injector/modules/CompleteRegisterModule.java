...
public class CompleteRegisterModule {
    ...
    @Provides
    @ActivityScope
    AutoLoginUseCase provideAutoLoginUseCase(AccountRepository accountRepository, @Named(ApplicationModule.UI_THREAD_NAMED) Scheduler uiThread, @Named(ApplicationModule.EXECUTOR_THREAD_NAMED) Scheduler executorThread){
        return new AutoLoginUseCase(uiThread,executorThread,accountRepository);
    }
