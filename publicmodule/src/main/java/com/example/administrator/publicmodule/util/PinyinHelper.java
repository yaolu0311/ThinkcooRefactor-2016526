...
/**
 * Created by cys on 2016/7/9 0009.
 */
public class PinyinHelper {

    private static String TAG = "PinyinHelper";

    private static int[] codeArray = new int[]{-20319, -20317, -20304, -20295,
    ...
    public static String[] pinyinArray = new String[]{"a", "ai", "an", "ang", "ao",
    ...
    public static Map<String, String> duoYinMap = new HashMap<String, String>();

    static {
        duoYinMap.put("郇", "huan");
        duoYinMap.put("单", "shan");
    }

    private StringBuilder buffer;
    private String resource;
    private static PinyinHelper mPinyinHelper = new PinyinHelper();

    public static PinyinHelper getInstance() {
        if (mPinyinHelper == null) {
    ...
    public String convert(String str) {
        String result = searchFromDuoyinMap(str);
        if (null != result) {
            return result;
        }
        ...
        if (ascii > 0 && ascii < 160) {
            ...
            for (int i = (codeArray.length - 1); i >= 0; i--) {
                if (codeArray[i] <= ascii) {
                    result = pinyinArray[i];
                    break;
                }
            }
        }
    ...
    private String searchFromDuoyinMap(String str) {
        return duoYinMap.get(str);
    }
     ...
     * 词组解析
     */
    public String getPinyins(String strArray) {
        String key, value;
        buffer = new StringBuilder();
        try {
            for (int i = 0; i < strArray.length(); i++) {
                key = strArray.substring(i, i + 1);
    ...
    public String getSpelling() {
        return this.getPinyins(this.getResource());
    }

    public String getFirstUpCaseLetter(String strArray) {
        String result = getPinyins(strArray);
        if (TextUtils.isEmpty(result) || !isAlphabet(result.charAt(0))) {
            return "#";
        }
        return String.valueOf(result.charAt(0)).toUpperCase();
    }

    private boolean isAlphabet(char str) {
        if (TextUtils.isEmpty(String.valueOf(str))) {
            return false;
        }
        int result = Integer.valueOf(str);
        return (65 <= result && result <= 90) || (97 <= result && result <= 122);
    }
}
